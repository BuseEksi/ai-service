"""
OpenAILLM sınıfı için unit testler.

Gerçek OpenAI API'sine hiçbir çağrı yapılmaz; app.llm.openai_llm.AsyncOpenAI
mock'lanarak client.chat.completions.create'in döneceği cevaplar simüle edilir.
"""
import json
...
def make_text_response(text: str):
    """choices[0].message.content dolu, tool_calls boş sahte bir OpenAI cevabı üretir."""
    message = SimpleNamespace(content=text, tool_calls=None)
    return SimpleNamespace(choices=[SimpleNamespace(message=message)])


def make_tool_call_response(tool_name: str, arguments: dict, call_id: str = "call_123"):
    """choices[0].message.tool_calls dolu sahte bir OpenAI cevabı üretir."""
...
@pytest.fixture
def llm():
    """AsyncOpenAI client'ı mock'lanmış bir OpenAILLM örneği döndürür."""
    with patch("app.llm.openai_llm.AsyncOpenAI") as mock_client_cls:
        mock_client = mock_client_cls.return_value
        mock_client.chat.completions.create = AsyncMock()
        instance = OpenAILLM(api_key="test-key", model="gpt-4o-mini")
        yield instance
...
class TestFormatToolsForOpenAI:
    def test_converts_anthropic_style_schema_to_openai_function_format(self, llm):
        ...
        result = llm._format_tools_for_openai(anthropic_style_tools)
    ...
    def test_missing_input_schema_defaults_to_empty_dict(self, llm):
        ...
        result = llm._format_tools_for_openai(tools)
...
class TestGenerateTextResponse:
    async def test_returns_plain_text_when_no_tool_called(self, llm):
        llm.client.chat.completions.create.return_value = make_text_response("merhaba, nasıl yardımcı olabilirim?")
    ...
    async def test_system_prompt_is_prepended_as_system_message(self, llm):
        llm.client.chat.completions.create.return_value = make_text_response("ok")
        ...
        sent_kwargs = llm.client.chat.completions.create.call_args.kwargs
...
class TestGenerateToolUseResponse:
    async def test_returns_anthropic_style_tool_use_dict(self, llm):
        llm.client.chat.completions.create.return_value = make_tool_call_response(
...
class TestGenerateRetryBehaviour:
    async def test_retries_on_timeout_then_succeeds(self, llm):
        with patch("app.llm.openai_llm.asyncio.sleep", new=AsyncMock()):
            with patch(
                "app.llm.openai_llm.asyncio.wait_for",
    ...
    async def test_raises_last_error_after_max_retries(self, llm):
        ...
        with patch("app.llm.openai_llm.asyncio.sleep", new=AsyncMock()):
            with patch("app.llm.openai_llm.asyncio.wait_for", side_effect=[boom, boom, boom]):
